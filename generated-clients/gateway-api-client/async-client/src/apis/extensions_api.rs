...
/// struct for typed errors of method [`resource_holders_page`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum ResourceHoldersPageError {
...
pub async fn resource_holders_page(configuration: &configuration::Configuration, resource_holders_request: models::ResourceHoldersRequest) -> Result<models::ResourceHoldersResponse, Error<ResourceHoldersPageError>> {
    let local_var_configuration = configuration;

    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str = format!("{}/extensions/resource-holders/page", local_var_configuration.base_path);
    let mut local_var_req_builder = local_var_client.request(reqwest::Method::POST, local_var_uri_str.as_str());

    if let Some(ref local_var_user_agent) = local_var_configuration.user_agent {
        local_var_req_builder = local_var_req_builder.header(reqwest::header::USER_AGENT, local_var_user_agent.clone());
    }
    local_var_req_builder = local_var_req_builder.json(&resource_holders_request);
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<ResourceHoldersPageError> = serde_json::from_str(&local_var_content).ok();
